 ...
 *
 *   // network
 *   CGwaycoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.gwaycoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CGwaycoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CGwaycoinNetwork(UniValue objIn)
    {
...
// // can be under: GwaycoinNetwork
// //   -- signature requirements : Key1(User)
// class CGwaycoinNetworkVariable : public CGovernanceObject
// {
// private:
