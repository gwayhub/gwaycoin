...
const char * const BITCOIN_CONF_FILENAME = "gwaycoin.conf";
const char * const BITCOIN_PID_FILENAME = "gwaycoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gwaycoin" is a composite category enabling all Gwaycoin-related debug output
            if(ptrCategory->count(string("gwaycoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Gwaycoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Gwaycoin
    // Mac: ~/Library/Application Support/Gwaycoin
    // Unix: ~/.gwaycoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gwaycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Gwaycoin";
#else
    // Unix
    return pathRet / ".gwaycoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gwaycoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gwaycoin.conf
