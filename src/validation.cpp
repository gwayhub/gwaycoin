...
#if defined(NDEBUG)
# error "Gwaycoin Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in gwees) are considered zero fee (for relaying, mining and transaction creation) */
...
void ThreadScriptCheck() {
    RenameThread("gwaycoin-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // GWAYCOIN CORE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(GWAYCOIN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(GWAYCOIN): couldn't find masternode or superblock payments"),
    ...
    if (founderPayment > 0) {
        ...
        if (!FounderPaid) {
            return state.DoS(0, error("ConnectBlock(GWAYCOIN): no founder reward"), REJECT_INVALID, "no-founder-reward");
        }
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // GWAYCOIN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(GWAYCOIN): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(GWAYCOIN): spork is off, skipping transaction locking checks\n");
    }
